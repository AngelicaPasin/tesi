...
#agente analista che elabora i dati grezzi e produce report_data
#non genera testo, ma si occupa solo di calcoli e logica
class AnalystAgent:
    ...
    def build_report_data(self, df_monitoraggio, df_ambientale, df_azioni, df_meteo, df_meteo_marine, metadata):
        soglie     = DatabaseClient.get_soglie(metadata["azienda_id"])
        benessere  = self._compute_benessere(df_monitoraggio, metadata.get("numero_pesci_iniziale"))
        ambientale = self._compute_ambientale(df_ambientale, soglie)
        meteo      = self._compute_meteo(df_meteo, df_meteo_marine, soglie)
        eventi     = self._compute_eventi(benessere, ambientale, meteo)
        azioni     = self._compute_azioni(df_azioni)

        #chain-of-table: ragionamento iterativo sui dati grezzi
        cot_insights = compute_chain_of_table_insights(
            df_monitoraggio,
            df_ambientale,
            df_meteo_marine,
            llm=self.llm
    ...
    def _compute_benessere(self, df, numero_pesci_iniziale=None):
        mortalita = self._compute_mortalita(df)
        return {
            "mortalita":     mortalita,
            "appetito":      self._compute_appetito(df),
            "comportamento": self._compute_comportamento(df),
            "sintomi":       self._compute_sintomi(df, numero_pesci_iniziale, morti_tot=mortalita["totale"])
        }

    #calcola le statistiche di mortalità (totale, media giornaliera e trend)
    def _compute_mortalita(self, df):
        ...
        trend = self._compute_trend(df["n_pesci_morti"])
    ...
    def _compute_appetito(self, df):
        ...
        if serie.empty:
            return {
                "valore_medio": None,
                "trend": "unavailable",
                "nota": "Score based on internal scale — interpret trend only"
            }

        media = serie.mean()
        trend = self._compute_trend(serie)
    ...
    def _compute_comportamento(self, df):
        totale = len(df)
    ...
    def _compute_sintomi(self, df, numero_pesci_iniziale=None, morti_tot=None):
        sintomi = {
    ...
    def _compute_ambientale(self, df, soglie):
    ...
    def _classify_livello(self, valore, livelli):
    ...
    def _compute_meteo(self, df_meteo, df_meteo_marine, soglie=None):
        soglie = soglie or {}
        result = {}
        ...
        def process_group(df, mapping):
            if df is not None and not df.empty:
                for res_key, (col, d_min, d_max, s_key) in mapping.items():
                    s = soglie.get(s_key, {})
                    result[res_key] = self._compute_parametro(
                        df, col, 
    ...
    def _compute_parametro(self, df, col, valid_range, livelli=None):
        #controllo colonna esistente
        if col not in df.columns:
            return self._empty_parametro()
        ...
        summary = self._summarize_series(cleaned)
        trend = self._compute_trend(cleaned)
        warning = self._compute_warning(raw, perc_outlier, valid_range)

        #classifica il valore medio in base ai livelli di allerta
        livello = self._classify_livello(summary["media"], livelli) if livelli else None
    ...
    def _summarize_series(self, series):
        return {
    ...
    def _compute_warning(self, raw, perc_outlier, valid_range):
        if perc_outlier > 0.2:
    ...
    def _compute_trend(self, series):
    ...
    def _compute_eventi(self, benessere, ambientale, meteo=None):
        eventi = []
