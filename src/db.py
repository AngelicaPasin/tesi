...
#centralizza la creazione dell'engine in un unico posto:
#così se cambiano le credenziali o il database, si modifica solo questo file (o il config.py) senza toccare il resto del codice.
...
engine = create_engine(DB_URI) #crea l'engine una sola volta quando il modulo viene importato
#l'engine non apre subito una connessione reale al db: la apre solo quando serve effettivamente eseguire una query (lazy connection)

def get_engine():
    return engine #restituisce l'engine già creato
    #tutti i moduli che hanno bisogno del db chiamano questa funzione 
    #invece di creare un engine proprio, evitando connessioni duplicate e spreco di risorse

def get_numero_pesci(gabbia_id, data_inizio):
    query = text("""
